...
export const cartRepository = {
    ...
    async getOrCreateCart(userId) {
        const existingCart = await query('SELECT * FROM cart WHERE user_id = $1', [userId]);
    ...
    async addItemToCart(cartId, productId, quantity) {
        ...
        const existingItem = await query('SELECT * FROM cart_items WHERE cart_id = $1 AND product_id = $2', [cartId, productId]);
        if (existingItem.rows.length > 0) {
            // Update quantity if exists
            const updatedItem = await query('UPDATE cart_items SET quantity = quantity + $1 WHERE cart_id = $2 AND product_id = $3 RETURNING *', [quantity, cartId, productId]);
            return updatedItem.rows[0];
        }
        else {
            // Add new item if not exists
            const newItem = await query('INSERT INTO cart_items (cart_id, product_id, quantity) VALUES ($1, $2, $3) RETURNING *', [cartId, productId, quantity]);
    ...
    async removeItemFromCart(cartItemId) {
        const result = await query('DELETE FROM cart_items WHERE id = $1 RETURNING id', [cartItemId]);
    ...
    async verifyCartItemOwnership(cartItemId, userId) {
        const result = await query(`SELECT ci.id 
       FROM cart_items ci
    ...
    async getCartItemsCount(userId) {
        const result = await query(`SELECT COUNT(ci.id) as items_count
       FROM cart_items ci
       JOIN cart c ON ci.cart_id = c.id
    ...
    async getProductInfo(productId) {
        const result = await query('SELECT id, name, price, stock, active FROM products WHERE id = $1', [productId]);
        return result.rows[0] || null;
    },
    // 🎯 Calculate cart total price
    async calculateCartTotal(userId) {
        const result = await query(`SELECT SUM(ci.quantity * p.price) as total
       FROM cart_items ci
       JOIN cart c ON ci.cart_id = c.id
