...
export const orderRepository = {
    ...
    async createOrderFromCart(userId, cartData, shippingInfo) {
        ...
        try {
            ...
            // 1. Create order
            const orderResult = await client.query(`INSERT INTO orders (user_id, total_amount, shipping_address, shipping_city, shipping_phone, status) 
         VALUES ($1, $2, $3, $4, $5, 'pending') 
         RETURNING *`, [userId, cartData.total_price, shippingInfo.shipping_address,
                shippingInfo.shipping_city, shippingInfo.shipping_phone]);
            const order = orderResult.rows[0];
            // 2. Add order items
            for (const item of cartData.items) {
                await client.query(`INSERT INTO order_items (order_id, product_id, quantity, price_at_time) 
           VALUES ($1, $2, $3, $4)`, [order.id, item.product_id, item.quantity, item.product_price]);
        ...
        catch (error) { // ⬅️ Add :any for error
    ...
    async getUserOrders(userId, page = 1, limit = 10, status) {
        ...
        const queryParams = [limit, offset, userId];
        let paramCount = 4;
        if (status) {
