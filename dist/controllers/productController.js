...
export class ProductController {
    ...
    static getProducts = catchAsync(async (req, res, next) => {
        ...
        const filters = {
            ...
            min_sales: req.query.min_sales ? parseInt(req.query.min_sales) : undefined,
            // 🔥🆕 أضف فلترة الأيام الجديدة هنا
            last_days: req.query.last_days ? parseInt(req.query.last_days) : undefined,
    ...
    static advancedSearch = catchAsync(async (req, res, next) => {
        const filters = {
            ...
            min_price: req.body.min_price || req.query.min_price ?
                parseFloat(req.body.min_price || req.query.min_price) : undefined,
            max_price: req.body.max_price || req.query.max_price ?
                parseFloat(req.body.max_price || req.query.max_price) : undefined,
            category_id: req.body.category_id || req.query.category_id ?
                parseInt(req.body.category_id || req.query.category_id) : undefined,
            page: req.body.page || req.query.page ?
                parseInt(req.body.page || req.query.page) : 1,
            limit: req.body.limit || req.query.limit ?
                parseInt(req.body.limit || req.query.limit) : 20
        };
        // تنظيف القيم
     ...
     */
    static getFilterOptions = catchAsync(async (req, res, next) => {
     ...
     */
    static getTopSelling = catchAsync(async (req, res, next) => {
     ...
     */
    static getProductsByGender = catchAsync(async (req, res, next) => {
     ...
     */
    static getProductsBySeason = catchAsync(async (req, res, next) => {
     ...
     */
    static getProductsByBrand = catchAsync(async (req, res, next) => {
    ...
    static updateProduct = catchAsync(async (req, res, next) => {
        ...
        if (updateData.sales_count)
            updateData.sales_count = parseInt(updateData.sales_count);
        // 🔥 معالجة الحقول الجديدة
        if (updateData.color !== undefined)
            updateData.color = String(updateData.color);
        if (updateData.size !== undefined)
            updateData.size = String(updateData.size);
        if (updateData.style !== undefined)
            updateData.style = String(updateData.style);
        if (updateData.brand !== undefined)
            updateData.brand = String(updateData.brand);
        if (updateData.gender !== undefined)
            updateData.gender = String(updateData.gender);
     ...
     */
    static updateProductImages = catchAsync(async (req, res, next) => {
        const productId = parseInt(req.params.id);
        // ⭐ UPDATED: الحصول على الملفات المرفوعة (مصفوفة)
        const uploadedFiles = req.files;
        let image_urls = [];
     ...
     */
    static getProductsByCategory = catchAsync(async (req, res, next) => {
     ...
     */
    static updateProductStock = catchAsync(async (req, res, next) => {
        const productId = parseInt(req.params.id);
     ...
     */
    static updateSalesCount = catchAsync(async (req, res, next) => {
        const productId = parseInt(req.params.id);
     ...
     */
    static getProductStats = catchAsync(async (req, res, next) => {
