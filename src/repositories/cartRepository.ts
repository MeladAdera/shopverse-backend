...
import { Cart, CartItem, CartWithItems } from '../models/Cart.js';
...
export const cartRepository = {
  ...
  async removeItemFromCart(cartItemId: number): Promise<boolean> {
    const result = await query(
  ...
  async verifyCartItemOwnership(cartItemId: number, userId: number): Promise<boolean> {
    const result = await query(
      `SELECT ci.id 
       FROM cart_items ci
  ...
  async getCartItemsCount(userId: number): Promise<number> {
    const result = await query(
      `SELECT COUNT(ci.id) as items_count
       FROM cart_items ci
       JOIN cart c ON ci.cart_id = c.id
       WHERE c.user_id = $1`,
      [userId]
