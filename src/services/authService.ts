...
export class AuthService {
  ...
  static async login(credentials: LoginCredentials): Promise<AuthResponse> {
    ...
    if (!user.active) {
      throw new AuthenticationError(
        'Your account has been blocked. Please contact the administrator to reactivate your account.'
      );
    }
  ...
  static async refreshToken(refreshToken: string): Promise<{ 
    ...
    if (!user.active) {
      throw new AuthenticationError(
        'Your account has been blocked. Tokens cannot be refreshed.'
      );
    }
  ...
  static async getProfile(userId: number): Promise<{
    ...
    if (!user.active) {
      throw new AuthenticationError(
        'Your account has been blocked. Profile access denied.'
      );
    }
   ...
   */
  static async checkUserStatus(userId: number): Promise<{ active: boolean }> {
